...
class MultiheadAttention(nn.Module):
    def __init__(self, num_heads, head_size, use_conv=False, kernel_size=3):
        ...
        if use_conv:
            # Replace one attention head with a convolution head
            self.heads = nn.ModuleList([Head(head_size) for _ in range(num_heads - 1)])
            self.conv_head = nn.Conv1d(n_embed, head_size, kernel_size=kernel_size)
        else:
